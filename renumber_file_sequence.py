...
outDir = "/Users/tcs6/Desktop/output"
image_extension = ".tif"
containString =  "RFP" # string that must be in the filename
numberToAdd = 10
...
def increment_filename(filename):
    ...
    # Search for the timepoint number in the base name: 3 digits after an underscore, at end of base name
    ...
    if match:
        ...
        incremented = str(int(number) + numberToAdd).zfill(3)
        # Replace the old number with the incremented one
        # --- construct the new file name using match and return it
        new_base = base[:match.start()] + "_" + incremented + base[match.end():]
        return new_base + ext
    else:
    ...
    for filename in files:
        ...
        if filename.endswith(image_extension):
            ...
            if os.path.isfile(old_path):
                ...
                shutil.copy(old_path, new_path) # safer than os.rename because old files are preserved
